...
**Session token delivery: cookie AND JSON body.** Both backends return the token/session secret in an `httpOnly` cookie (`res.cookie("token", ...)`) *and* in the JSON response body (`{ token: ... }`). Both auth middlewares accept either mechanism — `Authorization: Bearer <token>` header first, falling back to the `token` cookie — so the server is agnostic about which path the client uses.

In practice, the actual client uses the JSON body path exclusively. On login, `client/app.js` reads the token from `data.token` (line 159), stores it in `localStorage` (line 160, `localStorage.setItem("osdag_auth_token", currentToken)`), restores it on page reload (line 3, `localStorage.getItem("osdag_auth_token")`), sends it as `Authorization: Bearer` on every API call (e.g. line 224), and clears it on logout (line 211, `localStorage.removeItem`). The `httpOnly` cookie is never read by the client JavaScript — and in fact *cannot* work as the primary auth mechanism here, because the client is a static HTML file served from a different origin (or `file://`) than the backend. Cross-origin `fetch()` calls do not send cookies unless `credentials: "include"` is set on every request, and the client does not set this. So the cookie exists as a server-side fallback for same-origin deployments or tools like Postman that automatically handle cookies, not as the client's actual auth path.

**The XSS tradeoff is worth stating explicitly.** Storing the token in `localStorage` means any JavaScript running on the page (including injected XSS payloads) can read and exfiltrate it. An `httpOnly` cookie would not be readable by JavaScript at all, making it immune to XSS token theft. However, making the cookie path work for this cross-origin testing client would require adding `credentials: "include"` to every `fetch()` call, setting `SameSite=None; Secure` on the cookie (requiring HTTPS), and configuring CORS with a specific `Access-Control-Allow-Origin` (not `true`/wildcard) — all of which would complicate the reviewer's setup. The dual-delivery approach was chosen so the server supports both mechanisms and the client uses whichever one works for its deployment context, without requiring the reviewer to configure HTTPS or same-origin serving.
